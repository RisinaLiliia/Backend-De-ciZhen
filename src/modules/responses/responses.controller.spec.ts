...
describe('ResponsesService', () => {
  ...
  it('listMy uses aggregate and supports status filter', async () => {
    responseModelMock.aggregate.mockReturnValue(execWrap([]));
  ...
  it('listByRequestForClient uses aggregate and supports status filter', async () => {
    requestModelMock.findById.mockReturnValue(execWrap({ _id: '507f1f77bcf86cd799439011', clientId: 'c1' }));
    responseModelMock.aggregate.mockReturnValue(execWrap([]));

    await service.listByRequestForClient('c1', '507f1f77bcf86cd799439011', { status: 'pending' });

    const pipeline = responseModelMock.aggregate.mock.calls[0][0];
    expect(pipeline[0]).toEqual({ $match: { requestId: '507f1f77bcf86cd799439011', status: 'pending' } });
  });
