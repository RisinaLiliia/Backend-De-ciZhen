...
async function bootstrap() {
  ...
  const providerModel = app.get<Model<ProviderProfileDocument>>(
  ...
  const client = await ensureUser('demo-client@test.com', 'client');
  const p1 = await ensureUser('demo-provider-1@test.com', 'provider');
  const p2 = await ensureUser('demo-provider-2@test.com', 'provider');
  ...
  let req = await requestModel.findOne({ clientId }).exec();
  ...
  const ensureResponse = async (providerUserId: string) => {
    const exists = await responseModel
      .findOne({ requestId, providerUserId })
      .exec();
  ...
  console.log(`Client: demo-client@test.com / ${DEMO_PASSWORD}`);
  console.log(`Provider1: demo-provider-1@test.com / ${DEMO_PASSWORD}`);
  console.log(`Provider2: demo-provider-2@test.com / ${DEMO_PASSWORD}`);
